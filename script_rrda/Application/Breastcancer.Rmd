...
# chrom 13 only (CNV to expression)

Here, CNV is set to be X, gene expression is to be Y

```{r,eval=T}
base_path <- here("out", "BreastCancer")
da<-readRDS(here("RDAdata","breast.RDS"))

dir_path<-file.path(base_path, "13_rev")
...
plan(multisession)
cv_result<- rrda.cv(Y = Y, X = X,lambda = Lambda,maxrank = 40,nfold = 5,scale.X = F,scale.Y = F)
plan(sequential)
...
# all chromosome (CNV to expression)

```{r,eval=T}
base_path <- here("out", "BreastCancer")
da<-readRDS(here("RDAdata","breast.RDS"))

dir_path<-file.path(base_path, "all_rev")
