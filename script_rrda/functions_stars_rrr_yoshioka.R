...
# Only change rrr_ann_stars to rrr_ann_stars2 so that a preferable lambda range can be used (to be tested with the range of lambda of "rrda"). To avoid a native error in cases where no lambda is selected by the stability approach, the maximum value of lambda is returned. This implementaion is made by Yoshioka et al.
