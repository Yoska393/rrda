...
# 	Lambda <- exp(seq(8, 0, length.out = 50))
# 	
# 	criteria <- c("AIC", "BIC", "GIC", "BICP", "GCV", "StARS", "rrda")
# 	ranks_list <- list()
# 	
# 	for (mot in criteria) {
# 		if (mot == "rrda") {
...
# 													 nfold = 5, lambda = Lambda)
# 			optimized_rank <- cv_result$opt_min$rank
# 		} else if (mot == "StARS") {
# 			rrr_result <- rrr_ann_stars2(response = Y, X = X, lambda = Lambda)
# 			optimized_rank <- rrr_result$rank
# 		} else {
# 			rrr_result <- rrr2(Y = Y, X = X, penaltySVD = c("ann"),
# 												 ic.type = mot, modstr = rrr.modstr(lambda = Lambda))
# 			optimized_rank <- rrr_result$rank[which.min(rrr_result$ic)]
# 		}
# 		
# 		ranks_list[[mot]] <- optimized_rank
# 	}
# 	
# 	return(ranks_list)
# }
...
rank_evaluate_50_allcriteria<- function(n_iterations,rdasim,size){
