...
metname[186]<-"Guanosine-5"
metname[160]<-"Cyanidin-3-O"
...
top_x_idx <- order(x_scores, decreasing = TRUE)[1:38]  # 38 so that it becomes 30 variables afetr filtering

y_scores <- apply(b2, 1, function(row) sqrt(sum(row^2)))
top_y_idx <- order(y_scores, decreasing = TRUE)[1:39] # 39 so that it becomes 30 variables afetr filtering
...
y_scores <- apply(b2, 1, function(row) sqrt(sum(row^2)))
top_y_idx <- order(y_scores, decreasing = TRUE)[1:36] # 39 so that it becomes 30 variables afetr filtering
